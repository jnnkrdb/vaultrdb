...
func EnableEndpoint_ApiV1(r *mux.Router) {
	...
	r.Path("/api/v1/vault/buckets").Methods(http.MethodGet).Handler(server.DefaultMiddleware.ThenFunc(api_v1_buckets.TreeList))
	...
	r.Path("/api/v1/vault/buckets/{bucketpath}/sink").Methods(http.MethodPost).Handler(server.DefaultMiddleware.ThenFunc(api_v1_buckets_sink.Create))
	r.Path("/api/v1/vault/buckets/{bucketpath}/sink/{key}").Methods(http.MethodPut, http.MethodPatch).Handler(server.DefaultMiddleware.ThenFunc(api_v1_buckets_sink.Update))
